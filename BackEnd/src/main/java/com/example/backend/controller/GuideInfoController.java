...
public class GuideInfoController {
    ...
    public Response<GuideScheduledToursDto.Response> getGuidesScheduledTours(
            @PathVariable Long guideId, @PageableDefault(size = 6) Pageable pageable)
            throws IllegalAccessException {
        GuideScheduledToursDto.Response response = guideInfoGetService.guideScheduledToursService(
                guideId, pageable);
        return new Response<>("200", "성공적으로 가이드의 예정된 투어 목록을 가져왔습니다.",
                response);
    }
    ...
    public Response<GuideEndedToursDto.Response> getGuidesEndedTours(
        ...
        GuideEndedToursDto.Response response = guideInfoGetService.guideEndedToursService(guideId,
                pageable);
        return new Response<>("200", "성공적으로 가이드의 종료된 투어 목록을 가져왔습니다.",
                response);
    }

    @GetMapping("/guide/guideInfo/{guideId}")
    public Response<GuideInfoDto.Response> getGuideInfo(
    ...
    public Response<List<GuideFollowerDto>> getGuideFollower(
            @PathVariable Long guideId, @PageableDefault(size = 15) Pageable pageable) {
        List<GuideFollowerDto> response = guideInfoGetService.getGuideFollowerList(guideId, pageable);
    ...
    public Response<GuideCanceledToursDto.Response> getGuideCanceledTours(
        ...
        GuideCanceledToursDto.Response response = guideInfoGetService.getGuideCanceledTourList(
                guideId, pageable);
        return new Response<>("200", "성공적으로 가이드의 취소된 투어 목록을 가져왔습니다.",
                response);
    }
